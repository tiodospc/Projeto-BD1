...
CREATE TABLE SERIE(
	...
	FK_ID varchar(255) NOT NULL,
	CONSTRAINT   SERIE_pk  PRIMARY KEY ( FK_ID ), 
	constraint FK_ID_Categorias foreign key (FK_ID) references Categorias (ID_Categorias)
...
CREATE TABLE FILME(
     ...
     FK_ID  varchar(255) NOT NULL,
     CONSTRAINT  FILME_pk  PRIMARY KEY ( FK_ID ), 
     constraint FK_ID_Categorias foreign key (FK_ID) references Categorias (ID_Categorias)
);
...
insert into SERIE(Temporada, FK_ID)
        values('2 temporadas','F21'),
              ('5 temporadas','F22');
			  
insert into FILME(Bilheteria, FK_ID)
        values('375,2 milhões USD','F20'),
              ('470,7 milhões USD', 'F24');
